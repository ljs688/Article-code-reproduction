...
class GCFAggMVC(nn.Module):
    def __init__(self, view, input_size, low_feature_dim, high_feature_dim, device):
        ...
        self.TransformerEncoderLayer = nn.TransformerEncoderLayer(
            d_model=low_feature_dim*view, nhead=1, dim_feedforward=256, batch_first=True
        )
    ...
    def GCFAgg(self, xs):
        ...
        for v in range(self.view):
            ...
            z = self.encoders[v](x)
            zs.append(z)
        commonz = torch.cat(zs, 1)           # (B, low_feature_dim * view)

        # Build (B, B) structure matrix S via sample-wise cosine similarity
        # loss.py does S.repeat(2,2) -> needs (B, B) input
        z_norm = normalize(commonz, dim=1)   # (B, d_model)
        S = torch.matmul(z_norm, z_norm.T)  # (B, B), values in [-1, 1]
        S = (S + 1) / 2                      # rescale to [0, 1]

        # Transformer aggregation for the fused representation
        commonz_t = commonz.unsqueeze(1)     # (B, 1, d_model)
        commonz_t = self.TransformerEncoder(commonz_t)  # (B, 1, d_model)
        commonz_t = commonz_t.squeeze(1)     # (B, d_model)

        commonz_out = normalize(self.Common_view(commonz_t), dim=1)
        return commonz_out, S
